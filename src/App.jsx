    ...
    try {
      ...
      for (const perf of performances) {
        ...
        const perfRows = await sbPost(sbUrl, sbKey, "performances", [{
          ...
          subdistrict:      pd.subdistrict      || null,
          village:          pd.village          || null,
          venue:            pd.venue            || null,
          event_type:       pd.event_type       || null,
          period:           pd.period           || null,
          time_start:       pd.time_start       || null,
          time_end:         pd.time_end         || null,
          raw_line:         pd.raw_line         || "",
          confidence:       pd.confidence       ?? 0.5,
          flagged:          pd.flagged          || false,
          flag_reason:      pd.flag_reason      || null,
          is_drop_in:       pd.is_drop_in       || false,
          is_full_band:     pd.is_full_band     || false,
          is_retro:         pd.is_retro         || false,
          is_free_entry:    pd.is_free_entry    || false,
          validated:        false,
        }]);
